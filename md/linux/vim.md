...
## Start
`vim`  
`vim ch4ce.txt` ch4ce.txt存在就打开，不存在就新建
...
#### 选中文本
进入 Visual Mode，然后用 方向键 或 hjkl 选择文本  
`v` 按字符选择  
`V` 按行选择  
`CTRL＋v` 按块选择  

Visual Mode模式下，[]是选中到行首和行尾，{}是选中开头和结尾。
...
**注意！**
`"+`这类系统剪切板寄存器，只有GUI界面打开的 vim 才能使用，使用方法如下：  
检查 vim 版本是否支持与系统剪贴板的交互操作  
...
！！！SSH连接的VPS用以上方法是不行的，正常也不需要，直接用鼠标右键复制粘贴就行。  
因为VPS默认没有安装GUI，终端就是整个系统了，也就不存在系统剪切板，而连接VPS的SSH工具和VPS也没有数据交换的通道，也就不能实现 vps打开的vim 和 连接vps的电脑的数据进行复制和粘贴  
不过 vim 支持远程文件编辑`vim scp://user@host/PATH/TO/FILE`这样如果发生修改,还可以保存回去。在这种状态下,将vim的内容复制到系统剪贴板,以及复制到vim内部剪贴板都是没有问题的。  
除此之外，还有scp、rsync(同步,更为强大)、modem协议,sz,rz命令(这个目前securecrt还在使用,可以进行文件传输)、ftp服务以及相关的工具、http(这个是单向的)、其他几乎所有的文件服务方法。  
...
在启动 vim 时，当前用户根目录下的`.vimrc`文件会被自动读取，因此一般在当前用户的根目录下创建`.vimrc`文件。  
...
在.vimrc里加了一句 `mouse=v` 后就可以用鼠标选择复制
